module module_numerical_integration
  ...
  phi_y_times_phi_x, phi_y_times_phi_y
...
function coefficients_calculation(elem, trian) result(coeff)
  ...
  a = (y1 - y2)/(x0*y1 + x2*y0 + x1*y2 - x2*y1 - x0*y2 - x1*y0)
  b = (x2 - x1)/(x0*y1 + x2*y0 + x1*y2 - x2*y1 - x0*y2 - x1*y0)
  c = (x1*y2 - x2*y1)/(x0*y1 + x2*y0 + x1*y2 - x2*y1 - x0*y2 - x1*y0)
  
  !if ((x0*y1 + x2*y0 + x1*y2 - x2*y1 - x0*y2 - x1*y0) == 0d0) then
  
  !  print *, "ALARM", elem%nd_identificator, trian%identificator
