  ...
  real*8, allocatable :: lhs_a(:)
  integer, allocatable :: lhs_ia(:), lhs_ja(:)
...
! Arrays for the matrix sparse row (CSR) format
   ...
   type(Matrix) :: mass_matrix_high, mass_matrix_low, ND_mass_matrix, ND_mass_matrix_lum, Mass_VP, B_VP, &
   lhs_VP 
...
! Local variables
   ...
   real*8 :: resinit, prom, max_C_norm, L2_error      
...
! Local variables
   ...
   type(Matrix) :: M1, M2, M3, M4, Mr
   
   real*8 :: a_test(10), dia_test(10), a_test_res(10)
   integer :: ia_test(10), ja_test(10), ia_test_res(10), ja_test_res(10) 
   
   integer, parameter :: IRESTART = 20 
   integer, parameter :: mKryl = IRESTART+3
   integer :: ipKrylov
   integer :: ipaLU, ipjLU, ipjU, ipiw
   
   integer, parameter :: MH = IRESTART + 1, NH = IRESTART + 6 
   real*8 :: H(MH,NH)
   
   real*8 :: init_residual_norm
  ...
  number_of_non_Direchlet_elements = k - 1
   ...
   allocate(lhs_a(40*number_of_non_Direchlet_elements))
   allocate(lhs_ia(5*number_of_non_Direchlet_elements))
   allocate(lhs_ja(40*number_of_non_Direchlet_elements))
   
   
   lhs_ia(1) = 1
          
   do i = 1, number_of_non_Direchlet_elements*2
  ...
  do while (num < num_time_steps)
      ...
      !! calculate initial residual norm (result residual from previous global step)
      
      init_residual_norm = L2_norm(residual(ND_mass_matrix_lum, List_of_non_Direchlet_Elements, List_of_Triangles, &
            number_of_non_Direchlet_elements, number_of_triangles, time_step, 0), 2*number_of_non_Direchlet_elements)
            
      print *, "init residual norm:", init_residual_norm      
      
      
      do npic = 1, N_Picard
    
          !!! calculate new values of velocities, viscosities, delta, P 
    
         call velocity_delta_str_P_recalculation(List_of_non_Direchlet_Elements, List_of_Triangles, &
              number_of_non_Direchlet_elements, number_of_triangles, npic, 1)
          ...
          call aplb(number_of_non_Direchlet_elements*2, number_of_non_Direchlet_elements*2, 1, &
          Mass_VP%a, Mass_VP%ja, Mass_VP%ia, B_VP%a, B_VP%ja, B_VP%ia, lhs_a, lhs_ja, lhs_ia, &
          maxnz, iw, ierr)
          
          
          
          call lhs_VP%init(lhs_ia(number_of_non_Direchlet_elements*2+1)-1, number_of_non_Direchlet_elements*2)
          ...
          call VP_rhs_assembling(List_of_non_Direchlet_Elements, number_of_non_Direchlet_elements, rhs_VP, &
          ...
          ipaLU = 1
          ipKrylov = ipaLU+lhs_VP%nonzero
          ipjU  = 1
          ipjLU = ipjU+number_of_non_Direchlet_elements*2+1
          ipiw  = ipjLU+lhs_VP%nonzero !work array of length n

          call ilu0(number_of_non_Direchlet_elements*2,lhs_VP%a,lhs_VP%ja,lhs_VP%ia, &
           rW(ipaLU),iW(ipjLU),iW(ipjU),iW(ipiw),ierr)

        
          !set initial guess to 0
          call dcopy(number_of_non_Direchlet_elements*2,0d0,0,solution_vec,1)

          !compute initial residual norm 
          resinit = ddot(number_of_non_Direchlet_elements*2,rhs_VP,1,rhs_VP,1)
          ...
          INFO  = 0               
          NUNIT = 6               
          iprevec(1) = number_of_non_Direchlet_elements*2           
          imatvec(1) = number_of_non_Direchlet_elements*2             
          
          call slgmres( &
             prevec0, iprevec, iW,rW, &
             matvec,  imatvec, lhs_VP%ia, lhs_VP%ja, lhs_VP%a, &
             rW(ipKrylov), number_of_non_Direchlet_elements*2, mKryl, H, MH, NH, &
             number_of_non_Direchlet_elements*2, rhs_VP, solution_vec, &
             ITER, RESID, &
             INFO, NUNIT)
                        
          do i = 1, number_of_non_Direchlet_elements
           
            List_of_non_Direchlet_Elements(i)%pointing_element%u_old(1) = solution_vec(i)
            List_of_non_Direchlet_Elements(i)%pointing_element%u_old(2) = &
              solution_vec(number_of_non_Direchlet_elements + i)
          ...
          !  number_of_non_Direchlet_elements, number_of_triangles, time_step)/init_residual
          
          print *, "iteration", npic,"residual:",  &
            L2_norm(residual(ND_mass_matrix_lum, List_of_non_Direchlet_Elements, List_of_Triangles, &
            number_of_non_Direchlet_elements, number_of_triangles, time_step, 0), 2*number_of_non_Direchlet_elements)/ &
             init_residual_norm
