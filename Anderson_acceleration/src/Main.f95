...
! Allocating all variables
  ...
  allocate(solutions_anderson(m_Anderson, number_of_non_Direchlet_elements*2))
  allocate(residuals_anderson(m_Anderson, number_of_non_Direchlet_elements*2))
  allocate(g_anderson(m_Anderson, number_of_non_Direchlet_elements*2))
  allocate(init_resid(number_of_non_Direchlet_elements*2))
  allocate(new_sol(number_of_non_Direchlet_elements*2))
  
  solutions_anderson = 0d0
  ...
  do while (num < num_time_steps)
      ...
      do while(num_iter < m_Anderson)
        ...
        !! store solutions Anderson
        
        do i = 1, number_of_non_Direchlet_elements
        
          solutions_anderson(num_iter + 1, i) = List_of_non_Direchlet_Elements(i)%pointing_element%u_old(1)
          solutions_anderson(num_iter + 1, number_of_non_Direchlet_elements + i) = &
           List_of_non_Direchlet_Elements(i)%pointing_element%u_old(2)
        
        end do 
        
        
        !! store residuals, and g
        
        residuals_anderson(num_iter + 1, :) = &
              residual(ND_mass_matrix_lum, List_of_non_Direchlet_Elements, List_of_Triangles, &
              number_of_non_Direchlet_elements, number_of_triangles, time_step, 0)
           
        
        do i = 1, number_of_non_Direchlet_elements*2
        
          g_anderson(num_iter + 1, i) = solutions_anderson(num_iter + 1, i) - alpha_Anderson*&
                residuals_anderson(num_iter + 1, i)
        
        end do      
        
        !call print_resudal(resudal_mass, num)
      
        print *, "iter num", num_iter, "residual:", L2_norm( &
         residual(ND_mass_matrix_lum, List_of_non_Direchlet_Elements, List_of_Triangles, &
              number_of_non_Direchlet_elements, number_of_triangles, time_step, 0), &
          2*number_of_non_Direchlet_elements)/L2_norm(init_resid, 2*number_of_non_Direchlet_elements)
      ...
      do k = 1, num_anderson_iterations
      
        new_sol = 0d0
      
        new_sol = anderson_iterations_ls(residuals_anderson, g_anderson, number_of_non_Direchlet_elements)
        
        
        !do i = 1, 10
          
        !  print *, i, List_of_non_Direchlet_Elements(i)%pointing_element%u_resuid(1)
          
        !end do
        
        do j = 1, (m_Anderson - 1)
        
          residuals_anderson(j,:) = residuals_anderson(j+1,:)
          g_anderson(j,:) = g_anderson(j+1,:)
        
        end do
        
        do j = 1, number_of_non_Direchlet_elements
        
          List_of_non_Direchlet_Elements(j)%pointing_element%u_resuid(1) = &
          new_sol(j)
    
          List_of_non_Direchlet_Elements(j)%pointing_element%u_resuid(2) = &
          new_sol(number_of_non_Direchlet_elements + j)
        
        end do
        
        residuals_anderson(m_Anderson, :) = &
         residual(ND_mass_matrix_lum, List_of_non_Direchlet_Elements, List_of_Triangles, &
         number_of_non_Direchlet_elements, number_of_triangles, time_step, 1)
         
        g_anderson(m_Anderson, :) = new_sol - alpha_Anderson*residuals_anderson(m_Anderson, :)
        
        !! print residual
        
         print *, "iter num", num_iter + k, "residual:", L2_norm( &
         residual(ND_mass_matrix_lum, List_of_non_Direchlet_Elements, List_of_Triangles, &
              number_of_non_Direchlet_elements, number_of_triangles, time_step, 0), &
          2*number_of_non_Direchlet_elements)/L2_norm(init_resid, 2*number_of_non_Direchlet_elements)
